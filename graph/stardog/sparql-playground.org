...
*** Healthcheck
    ...
    Tell CURL to print HTTP status code
    #+begin_src bash :var url="http://localhost:5820"
...
** Very basic RDFS Inference
   ...
   #+end_src
* All Paws Time Series and Employment Contracts for [2023-03-10 Fri]
...
* Org Mode Parsed
  #+name: dblist
  #+begin_src emacs-lisp :dir /docker:stardog@stardog:
    (shell-command-to-string "/opt/stardog/bin/stardog-admin db list")
  #+end_src

  #+name: dbnamespace
  #+begin_src emacs-lisp   :dir /docker:stardog@stardog: :var db="orgmode"
    ;; (if (and (boundp 'dbnamespace) dbnamespace)
    ;;     dbnamespace
...
** Let's play
   #+begin_src sparql
     select * where {graph ?g  {?s ?p ?o}} order by rand() limit 10
   #+end_src

   #+begin_src sparql
     select *
     from <urn:orgmode:homelab2>
     where {
       ?topic a :Topic ;
              :topic_headline "Drink" ;
              :from_file ?file ;
              :topic_level ?start_level ;
             ^:child_of ?child .
       ?child ?p ?o .
     }
     order by rand()
   ...
   #+begin_src sparql
     select distinct ?label from <urn:orgmode:homelab2> {?topic a :Topic ; ?topic_level 1 ; rdfs:label ?label} limit 10000
   #+end_src
...
** Query and explore results
   ...
   #+end_src
* Python!
  ref [[https://sparqlwrapper.readthedocs.io/en/latest/main.html][SPARQL Endpoint interface to Python — SPARQLWrapper documentation ≫ sparqlwrapper.readthedocs.io]]
  #+begin_src python
    # SPARQLWrapper2 is only for JSON results
    from SPARQLWrapper import SPARQLWrapper, JSON, SPARQLWrapper2

    # sparql = SPARQLWrapper( "http://localhost:5820/northwind/query")
    sparql = SPARQLWrapper2( "http://localhost:5820/northwind/query")
    sparql.setReturnFormat("json") # csv, tsv, json
    sparql.setCredentials("admin", "admin")
    sparql.setQuery("select * {?s ?p ?o} limit 10")

    try:
      # ret = sparql.queryAndConvert()
      # print(ret) # for csv and tsv
      ret = sparql.query()
      # for r in ret["results"]["bindings"]:  # for JSON results
      #   print(r)
      d = ret.convert()
