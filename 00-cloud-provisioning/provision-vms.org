   ...
   + SSH to our new server node to install =kube-vip=
     + Download RBAC config
       #+begin_src bash
         curl -s https://kube-vip.io/manifests/rbac.yaml > \
              /var/lib/rancher/k3s/server/manifests/kube-vip-rbac.yam
       #+end_src
     ...
     + Pull =kube-vip= image
       ...
       #+begin_src bash
         ...
         alias kube-vip="ctr run --r m--net-host docker.io/plndr/kube-vip:0.3.2 vip /kube-vip"
       #+end_src
     + Create the manifest
