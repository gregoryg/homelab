...
***    The cownonical Cow Me! script
   #+begin_src bash :tangle ~/bin/cowme :results replace output
     ...
     if [ -z ${mycow} ] ; then
         IFS=',' read -r -a safe_cows <<< 'default,default,default,default,bud-frogs,duck,elephant,elephant-in-snake,three-eyes,tux,rancher,rancher-trademarked,rancher-k3os,rancher-k3os-trademarked,kitten,robotfindskitten,owl,hellokitty,pony-smaller,unipony-smaller,moofasa,skeleton,www'
...
**** Working together with Org Mode
     ...
     #+begin_src clojure :tangle ~/.logseq/config/config.edn
