...
* Literate document setup
  ...
  For some interesting examples --  including getting into complexities of chaining multiple code blocks, see [[http://www.howardism.org/Technical/Emacs/literate-programming-tutorial.html][Introduction to Literate Programming]] by Howard Abrams
...
* Home Lab resources and inspiration
  + /r/homelab
  + /r/selfhosted
...
*** The cownonical Cow Me! script
   #+begin_src bash :tangle ~/bin/cowme :comments both :results replace output
     # Optionally specify a cow - whether or not in the safe list
     mycow=$1
     # cowfiles are in different paths on openSUSE, Debian and Ubuntu, so include them all
     export COWPATH=/usr/share/cows:/usr/share/cowsay/cows:~/projects/homelab/cows:/usr/share/games/fortunes:/usr/share/games/fortunes-de
     if [ -z ${mycow} ] ; then
         IFS=',' read -r -a safe_cows <<< 'default,default,default,default,bud-frogs,duck,elephant,three-eyes,tux,rancher,rancher-trademarked,rancher-k3os,rancher-k3os-trademarked,kitten,robotfindskitten,owl,hellokitty,pony-smaller,unipony-smaller,moofasa,skeleton,www'
         mycow=${safe_cows[$((RANDOM % ${#safe_cows[@]}))]}
     fi
     # Choose fortune cookie files based on selected cow
     case ${mycow} in
         rancher*|tux|chameleon)
             db='linux computers debian science startrek'
             ;;

         owl|satanic|eyes)
             db='cookie definitions steven-wright deep-thoughts zippy mitch-hedberg'
             ;;
         ,*)
             db=
     esac
     ...
     if [[ $(command -v fortune 2>/dev/null) && $(command -v cowsay 2>/dev/null) ]] ; then
         ...
         f=$(fortune -c -e ${db})
         # split source and content when using -c parameter
         cookiefile=$(echo $f | cut -d'%' -f1)
         fcontent=$(echo $f | cut -d'%' -f2-)
         echo $fcontent | ${cowcmd} -f ${mycow} -${cowopts[$((RANDOM % ${#cowopts[@]}))]}
         echo ${cookiefile}
         echo
     fi
   #+end_src

*** Links may be needed to get all fortunes
    It's harder to pass multiple paths to =fortune= (at least on Debian), so links to
    additional fortune files may be advised.

    #+begin_src bash :results none
      sudo ln -s ~/projects/homelab/fortunes/* /usr/share/games/fortunes/
    #+end_src

*** Sample command to index fortune text files (add =.dat= file)
    #+begin_src bash
      strfile -c % mitch-hedberg mitch-hedberg.dat
    #+end_src
** List all docker tags for a repo
   #+begin_src bash :tangle ~/bin/docker-tags.sh :comments both
     if [ $# -lt 1 ]
     then
         cat << HELP
     ...
     namespace=$(echo "${1}" | cut -s -d'/' -f1)
     image=$(echo "${1}" | cut -s -d'/' -f2)
     if [ -z "${namespace}" ]; then
         namespace="library"
         image="${1}"
     fi

     tags=$(curl --silent "https://registry.hub.docker.com/v2/repositories/${namespace}/${image}/tags?page_size=1000" | jq -r '.results[].name' | sort --version-sort)
...
*** try segmented chapter detection
**** Flow
     1. pass in URL to video that can be processed by =yt-dlp=
        + N.B. we start with assumptions that may be relevant only to YouTube URLs
     2. Detect whether the video has sections/chapters
     3. Initiate transcription of single video or video chapters using =whisper=
     4. Format the transcript into paragraphs with the =wtpsplit= library
     5. Generate a Markdown formatted transcript with video title and section titles
     6. Sign off and call it a day
**** TODO Improvements [0/2]
***** TODO Maintain file paths for funky paths that include shell-sensitive characters
***** TODO Get certain vars/vals set for sane usage
      + path of downloaded "parent" =.mp3= audio file
        + this matches ='%(title)s/%(title)s.%(ext)s'= from =yt-dlp=
**** Python code
     ...
     + [ ] TODO: trim spaces before joining sentences (eliminate double-space between words in a sentence)
     #+begin_src python :tangle scripts/yt-audio.py :shebang #!/home/gregj/.local/python-venvs/ytaudio/bin/python3
       import os
       import subprocess
       import json
       import argparse
...
***** Tests for chaptered video

**** Bash script
     This shell script is  deprecated in favor of the python script
    #+begin_src bash :results verbatim :tangle no
      ...
      function transcribe_audio_file() {
          # Send audio to whisper to produce transcription
          # $1 is audio file path - NOTE: path is shell-escaped already
          xaudiopath=$1
          xaudiodir=$(dirname ${xaudiopath})
          echo "  Audio extraction of ${xaudiopath}"
          echo "    command will be rsync -av ${xaudiopath} ${SSH_USER}@${SSH_HOST}:${xaudiopath}"
          if [ -n "${SSH_HOST}" ]; then
              # use rsync to create new directories on target system
              rsync -av ${xaudiopath} ${SSH_USER}@${SSH_HOST}:/tmp/transcribedir/
              WHISPER_CMD="ssh ${SSH_USER}@${SSH_HOST} "
          fi
          WHISPER_CMD="${WHISPER_CMD} /home/${SSH_USER}/.local/bin/whisper --task transcribe --model ${WHISPER_MODEL} --word_timestamps True --output_format all --output_dir /tmp/transcribedir /tmp/transcribedir/$(basename ${xaudiopath})"
          echo "  ${WHISPER_CMD}"
          ${WHISPER_CMD}
          return
      }

      function format_transcript() {
          # $1 is a plain text file
          # $2 is optional section title
          # this function writes a markdown file with sentence and paragraph formatting
          # output of script must be captured in the lovely global var concept that bash provides us
          FORMAT_CMD="~/.local/python-venvs/wtpsplit/bin/python ~/gort.py ${1}"
          formatted_transcript=$(ssh ${SSH_USER}@${SSH_HOST} ${FORMAT_CMD})
          return
      }
      ...
      safepath=$(printf '%q' "${audiopath}")
      infofile=$(ls $(dirname ${audiopath})/*.info.json|head -1)
      numchapters=$(jq -r '.chapters | length' ${infofile})
      if [[ ${numchapters} == 0  ]] ; then
          echo "there ain't no chapters in this one"
          echo "TODO: transcribe unchaptered video file ${audiopath}"
          transcribe_audio_file "${safepath}"
      else
          indexmax=$((${numchapters} - 1))
          echo "there are ${numchapters} chapters in this video"
          for indexnum in $(seq 0 ${indexmax}); do
              chapnum=$((${indexnum} + 1))
              chaptitle=$(jq -r ".chapters[${indexnum}] | .title" ${infofile})
              chapfile=$(ls $(dirname ${safepath})/\[$(printf '%02d' ${chapnum})\]*mp3|head -1)
              # chapfile=$(printf '%q' "${chapfile}")
              echo "Chapter ${chapnum}: ${chaptitle} - file ${chapfile}"
              echo "Transcribing... ${chapfile}"
              transcribe_audio_file ${chapfile}
              format_transcript /tmp/transcribedir/$(basename ${chapfile} | sed 's,mp3$,txt,')
              echo "## ${chaptitle}" >> /tmp/gort.md
              cat "${formatted_transcript}" >> /tmp/gort.md
          done
      fi
    #+end_src
**** un-chaptered video
     + Screen Junkies - Honest Trailers Alien Romulus
     #+begin_src bash :results verbatim
       yt-audio.py 'https://www.youtube.com/watch?v=eoFlbna9-cY'
#+end_src

**** chaptered video
     + Inteligencia Artificial - 🚨 NOTICIAS IA 🚨: ADOBE lo peta en audiovisual 🤯💥
     #+begin_src bash :results verbatim
     yt-audio.py 'https://www.youtube.com/watch?v=nbSRBs0EMlE'
#+end_src
...
*** Apache Kubernetes using Kubeadm
    + [[file:kubernetes/01-k8s-distribution/apache-kubeadm/]]
*** Rancher Labs' RKE
    + [[file:kubernetes/01-k8s-distribution/rancher-rke/]]
    + Have a look at the two canonical starter configs from Rancher:
      + [[file:kubernetes/01-k8s-distribution/rancher-rke/templates/cluster-full-example.yaml-TEMPLATE][Cluster full example config]]
      + [[file:kubernetes/01-k8s-distribution/rancher-rke/templates/cluster-minimal-example.yaml-TEMPLATE][Cluster minimal example config]]
    + [[file:kubernetes/01-k8s-distribution/rancher-rke/cluster.yaml][My specific homelab config]]
...
** Set up Load Balancing and Ingress Controller
   ...
   + Give MetalLB a pool of IPs
     + Here I'm using a pool from the primary home network
       #+begin_src yaml :tangle manifests/metallb-pool.yaml :mkdirp yes
       ...
       #+end_src
       #+begin_src emacs-lisp :tangle manifests/metallb-l2advert.yaml :mkdirp yes
         apiVersion: metallb.io/v1beta1
         ...
         data:
           config: |
             ...
             - name: default
               ...
               - 172.16.17.230-172.16.17.250
       #+end_src
*** HELM Method for MetallLB
    + First have a =values.yaml= file ready
      #+begin_src yaml :tangle /tmp/metallb-values.yaml
        configInline:
          address-pools:
          - name: default
            protocol: layer2
...
** Clean up after RKE
   ...
   #+end_src
* k3s/k3sup
  As of OpenSSH server 9.x, there is a change needed to make SSH work with k3s
  in =/etc/ssh/sshd_config= make sure you have the following settings
  #+begin_src conf
...
* Storage
** NFS
   + ref: [[https://github.com/PhilipSchmid/k8s-home-lab#persistent-storage-using-nfs-subdir-external-provisioner][GitHub - PhilipSchmid/k8s-home-lab: Setup for a K8s home lab running on a sin...]]
   + =nfs-common= must be installed on all k8s nodes
     #+begin_src yaml :tangle /tmp/nfs-values.yaml
       nfs:
         server: 172.16.17.5
         path: /volume3/ISOs
...
* Setting default applications in the absence of a Desktop Environment
  Using a tiling window manager such as EXWM, you don't have the window dressing of
  graphical utilities to set default apps.  Here are some steps to setting those things at
  a lower level
** Use =alternatives= for =x-www-browser= and =gnome-www-browser=
   #+begin_src bash
     sudo update-alternatives --config /usr/bin/x-www-browser x-www-browser /usr/bin/microsoft-edge 10
   #+end_src
** Keep mime types up to date
   In =~/.config/mimeapps.list=
   #+begin_src bash
     [Default Applications]
     x-scheme-handler/http=microsoft-edge.desktop
   #+end_src
** Update using =xdg-settings=
   #+begin_src bash
     xdg-settings set default-web-browser microsoft-edge.desktop
   #+end_src
* SSL/TLS - the bane of my existence
  Why is this so hard?  Let's make it easier
  + ref: [[https://www.withoutthesarcasm.com/letsencrypt-for-private-networks/][Let's Encrypt for Private Networks - Without The Sarcasm]]
  + [[https://github.com/acmesh-official/acme.sh/wiki/dnsapi][dnsapi · acmesh-official/acme.sh Wiki · GitHub]]
    + I use Cloudflare, so this seems the best option
  + Cloudflare's =origin-ca-issuer=
    + [[https://github.com/cloudflare/origin-ca-issuer][GitHub - cloudflare/origin-ca-issuer]]
  + The following steps worked [2022-05-31 Tue]
    + On the Rancher cluster
      ...
      + Install Rancher with cert-manager as ingress
        ...
        #+end_src
    + Chrome may need cookies cleared, restart or whatever - try in incognito tab if you
      get "certificate is valid, but site is unsafe"
...
** Buzzing from headphone jack when nothing is playing
   ...
   + On openSUSE Tumbleweed
     ...
     #+end_src
** Prevent USB microphones from suspending
   + ref: [[https://askubuntu.com/a/1311891/104280][20.04 - USB webcam microphone not working - Ask Ubuntu]]
   The problem: USB devices may be set to suspend after being idle

   The fix: in =/etc/pulse/default.pa= comment out the line reading =load-module
   module-suspend-on-idle=

   Test using =pacmd list-sources=
* Printer shenaniganas
** Find the dang printer
  ...
  #+end_src
** Set printer to print pages in reverse order
  #+begin_src bash
...
* Firefox touchscreen scrolling
  + Ref: [[https://askubuntu.com/a/994483/104280][Ubuntu 16 and touchscreen scrolling in FireFox - Ask Ubuntu]]
  + Change config param in Firefox from 2 to 1
    about:config
    param: dom.w3c_touch_events.enabled=1
  + Add env var to (believe it or not) =/etc/security/pam_env.conf=
...
** Debian and Ubuntu deprecated =apt-key= --  follow these miserable steps instead
   ...
   + At this point, nothing has changed and apt doesn't know the key exists. The last step
     ...
     repo.
** Keep Ibus off my damn systems
   :PROPERTIES:
   :ID:       a6737084-b8d4-4050-86bc-10a394cfb81d
   :END:
   Ibus is the horrible thing that makes the letter =e= appear in dired when I type
   =C-;= - its stunning usefulness is allowing you a shortcut to enter any of several
   thousand emojis.

*** First choice - remove it
    Only works if it doesn't remove programs you want that depend on it
    #+begin_src emacs-lisp
      sudo apt purge ibus
    #+end_src
*** Second and maybe best choice - keep Ibus but change settings to rid yourself of the emoji key
    ...
    #+end_src
*** Tiresome but purist choice: remove dependency in Debian package
   Zoom packages this as a dependency - which is quite idiotic.

   If there is no software you care about that depends on ibus, you can simply do
   =sudo apt purge ibus=

   Otherwise, plead with package maintainers to remove the dependency.

   In the case of Zoom that approach has so far proven fruitless, so remove the dependency
   in the =.deb= package as explained here:

   [[https://hashman.ca/zoom/][hashman.ca :: Repack Zoom .debs to remove the `ibus` dependency]]

   #+begin_src bash :tangle /tmp/de-ibusinize-zoom.sh
     scratch=$(mktemp -d)
   ...
   #+end_src
* MySQL / MariaDB on Kubernetes
  + ref: [[https://artifacthub.io/packages/helm/bitnami/mariadb][mariadb 11.4.2 · bitnami/bitnami]]
  #+begin_src bash
...
* Python Virtual Environments
  ...
  + Ref: [[https://docs.python.org/3/tutorial/venv.html][12. Virtual Environments and Packages — Python 3.10.2 documentation]]
** Create directory for venvs under =.local=
   #+begin_src bash
     mkdir -p ~/.local/python-venvs/
   #+end_src
** Create and activate venv from CLI
   #+begin_src bash
   ...
   #+end_src
** Emacs
   Use the Emacs =python-venv= package (ref my Emacs setup in [[https://github.com/gregoryg/emacs-gregoryg/blob/master/README.org#python-mode-setup][Python Mode Setup ≫ github.com]]
*** Use Emacs package to create and activate venvs
   M-x pyvenv-create
   M-x pyvenv-workon
** Setup basic virtual environments for specific uses
   =pip-search= is a given for all environments
*** myspark for running Jupyter and local Spark
    #+begin_src conf :tangle ~/.local/python-venvs/myspark-requirements.txt
      configparser
      findspark
      google-auth
      ...
      pystardog
    #+end_src
*** gmail for use of the Google Gmail API
    #+begin_src bash :tangle ~/.local/python-venvs/gmail-requirements.txt
      beautifulsoup4
      bs4
      cachetools
      certifi
...
*** databricks
    ...
    #+begin_src conf :tangle ~/.local/python-venvs/databricks-requirements.txt
      databricks-cli
      databricks-connect
      pip-search
    #+end_src
*** langchain-llm
    This environment can grow huge quickly and will merit careful management: uninstall
    outdated methods & APIs for example
    #+begin_src conf :tangle ~/.local/python-venvs/langchain-llm-requirements.txt
      streamlit
      python-dotenv
      faiss-cpu
      langchain
...
* R kernel in Jupyter
  + [[https://richpauloo.github.io/2018-05-16-Installing-the-R-kernel-in-Jupyter-Lab/][Installing the R kernel in Jupyter Lab]]
    #+begin_src bash
...
* PySpark and Jupyter notebook
  + ref: [[https://www.sicara.ai/blog/2017-05-02-get-started-pyspark-jupyter-notebook-3-minutes][Get Started with PySpark and Jupyter Notebook in 3 Minutes | Sicara]]
** Pre-reqs
...
** Use Pyspark in Jupyter
   ...
   #+end_src
** Test connection TigerGraph - read a Vertex type
   #+begin_src python
     jdbcDF = spark.read \
       .format("jdbc") \
     ...
     jdbcDF.show()
   #+end_src
* Graph databases and Graph Thinking
** Databases
*** Neo4j
...
*** Quine
    ...
    + ref [[https://quine.io/docs/getting-started/quick-start][Quick Start]]
    +
*** Stardog
**** Helm install
     #+begin_src bash
     ...
     #+end_src
**** SPARQL query API with LSP
     + Install LSP server ref: [[https://github.com/stardog-union/stardog-language-servers][GitHub - stardog-union/stardog-language-servers: Language Servers for Stardog...]]
       #+begin_src bash
     ...
     + Test using music database
       #+begin_src sparql :url http://172.16.17.16:5820/music/query/
         SELECT ?s ?song
         WHERE {
           ?s :sings ?song .
         }
...
**** Installer script for the tigergraph-k8s image
...
******* Try a bunch of queries
        + Sample queries for this module
          ...
          [[file:graph/tigergraph/udemy/documents/InstaFollow+Sample+Queries.sql]]
...
****** Knowledge test
       + 95%
** Thinking / Note taking / Visualization
*** Logseq
**** Working together with Org Mode
     + Best global defaults
       Tangle to ~/.logseq/config/config.edn
     #+begin_src clojure
...
**** Try some of the 3rd-party modules
     #+begin_src python
...
**** A small fake aerospace project data set
     #+begin_src ruby
       require 'csv'
       require 'faker'
...
** Common Lisp
*** The amazing Kandria
   [[https://reader.tymoon.eu/article/413][An Overview of Kandria's Development with Lisp - 妖怪世捨て人 ≫ reader.tymoon.eu]]
*** Radiance - A web application framework in Common Lisp
    [[https://shirakumo.github.io/radiance/][Radiance ≫ shirakumo.github.io]]
    [[https://github.com/Shirakumo/radiance][Shirakumo/radiance: A Common Lisp web application environment ≫ github.com]]
...
*** Setup on Debian
    ...
    #+end_src
*** Try out [[https://www.gnu.org/software/g-golf/index.html][G-Golf]] for GUI development
    #+begin_src bash
...
** Racket
   :PROPERTIES:
   :ID:       e69b8b8e-0ac3-4ac2-a687-307bb8931d0b
   :END:
   #+begin_src bash
...
** swat! - the amazing tty fly swatting game
   #+begin_src c :tangle swat2.cc
     #include <stdio.h>
     #include <math.h>
     ...
     void cheat(int xnew, int ynew, int digits) {

     }

     int ppos(xuser, yuser, xfly, yfly)
          int *xuser, *yuser, *xfly, *yfly;
     {
       int visible;
       at(*yfly, *xfly);
       printf("%%");
       visible = (0 <= *xuser) && (*xuser <= xmax) && (0 <= *yuser) && (*yuser <= ymax);
       if (visible) {
         at(yuser, xuser);
         printf("*");
       }
       ...
       int *xfly, *yfly, *xuser, *yuser;
     {
       int i, hops;
       ftime(&time_detail);
       srand(time_detail, millitm);
       hops = RND(19);
       for (i = 1; i <= hops; i++) {
         at(*yfly, *xfly);
         printf("%c", (*xfly == *xuser) && (*yfly == *yuser) ? '*' : ' ');
         ,*xfly += RND(3) - 2;
         ,*yfly += RND(3) - 2;
         if (*xfly < 0) *xfly = 0;
         if (*yfly < 0) *yfly = 0;
         if (*xfly > xmax) *xfly = xmax;
         if (*yfly > ymax) *yfly = ymax;
         if (!slow || i == hops) {
           at(*yfly, *xfly);
           printf("%c", (*xfly == *xuser) && (*yfly == *yuser) ? '*' : ' ');
         }
     ...
     void cheat(xnew, ynew, digits)
          int xnew, ynew, digits;
     {
       int j, i, xcrt, ycrt;
       for (j = -1; j <= 1; j++) {
         for (i = -1; i <= 1; i++) {
           xcrt = xnew + i;
           ycrt = ynew + j;
           if ((0 <= xcrt) && (xcrt <= xmax) && (0 <= ycrt) && (ycrt <= ymax)) {
             at(ycrt, xcrt);
             if (digits)
               printf("%d", 1 + 5 - (3 * j));
             else
     ...
     void at(row, col)
          int row, col;
     {
       static char arr[] = {11, 0, 16, 0, 0};
       arr[1] = 64 + (row % 24);
       arr[3] = ((col % 80) / 10) * 16 + col % 10;
       printf("%s", arr);
     } // at

     void phelp() {
       int dummy;
       printf("%c\n\n\nThe basic point of the game is to swat that damn fly", CS);
       printf("\n(i.e., the percent sign).  You control your direction by inputting");
       printf("\nnumbers in the standard number pad configuration.");
       ...
       do
         {
           ...
           do { /* loop each move */
             ...
             // fly hops
             hop(&xfly, &yfly, &xuser, &yuser);
             at(ymax,1);
             printf("%sYou're at [%d,%d], ", eos, xuser, yuser);
             if (cheating) printf("cheating, ");
             if (tracing) printf("tracing, ");
             printf("fly's at [%d,%d]\n", xfly, yfly);
             printf("Your move? (Dir 1-9, h, c, q, r, t)");
             dir = getc(stdin);
             steps++;
             if (cheating) {
               digits = 0;
               cheat(xnew, ynew, digits);  /* erase hints */
             }
             switch(dir) {
             case '1':
               xvel--;
