    ...
    #+begin_src bash :results verbatim :tangle /tmp/yt-audio.sh
      ...
      if [[ ${numchapters} == 0  ]] ; then
          ...
          for indexnum in $(seq 0 ${indexmax}); do
              ...
              format_transcript /tmp/transcribedir/$(basename ${chapfile}).txt
              # ssh ${SSH_USER}@${SSH_HOST} .local/python-venvs/wtpsplit/bin/python gort.py /tmp/transcribedir/$(basename "${chapfile}".txt)
          done
      fi
    #+end_src
