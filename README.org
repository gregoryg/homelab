...
* Graph database(s)
** Tigergraph
*** STARTED Learn Tigergraph on Udemy
    DEADLINE: <2021-06-19 Sat 18:55>
    :LOGBOOK:
    CLOCK: [2021-06-27 Sun 12:14]
...
***** SSH to Tigergraph server
      ...
      #+begin_src emacs-lisp
        (call-process-shell-command "ssh -o StrictHostKeyChecking=no -p 14022  tigergraph@localhost id")
        (cd "/ssh:tigergraph@localhost#14022:")
        (shell "tigergraph-sh")
      #+end_src
***** Start Tigergraph
...
**** GSQL - Advanced - Saved Queries
     + Named query syntax
       ...
       [[file:databases/tigergraph/udemy/img/tigergraph-udemy-named-query-syntax.png]]
...
**** Graph Studio - explore graph
     transcript: [[file:databases/tigergraph/udemy/documents/GraphStudio-exploregraph.org]]
...
**** Questions for Steven
     + What automation is common for creating graphs and loading data (Terraform for
       example)
       + Related: what use of cloud platforms are common for the field?   and for customers
         (vSphere for example)
     + what steps are being taken   towards Kubernetes implementation in distributed processing
     + how can I prepare for Tigergraph certification
     + Neo4j is way ahead on Kubernetes deployment - with Helm charts even.  What is the
       timeline for getting k8s horizontal scale out deployment?
     + The images for Tigergraph seem to be quite large
     + images are massive - what disk and memory in laptops do y'all provide?
     + API Server runs on port 9000 - can require authentication with use of bearer token.
       What higher level auth is supported (Okta, AD etc?)
     + Are you going to stick with this company?  What's the growth plan?
     + in-person interview?
     + How well does the RDBMS importer work?  Are there common mismatches with good graph
       db design coming from RDBMS?
     + How do you start designing while thinking graphs?  What makes a good edge -
       e.g. artist composes songs vs. song is-composed-by artist.
     + Why can I not get interactive =gsql= to accept multiline CREATE statements?
     + OSS: almost everything is proprietary, even gsql - is there any significant
       contribution to OSS 
* Home automation
** Home Assistant
