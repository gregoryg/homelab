...
*** Remove dependency in Debian package
   ...
   In the case of Zoom that has so far proven fruitless, so remove the dependency in the
   =.deb= package as explained here:
   ...
   #+end_src
*** OR keep Ibus but change settings to rid yourself of the emoji key
    Do it via gui: =ibus-settings=, =emoji= tab

    Do it via command line (untested):
    #+begin_src bash
