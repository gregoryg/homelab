...
** Cow me, baby
   ...
   #+begin_src bash :tangle ~/bin/cowme :results replace output
     ...
     mycow=$1
     export COWPATH=/usr/share/cows:~/projects/homelab/cows
     if [ -z ${mycow} ] ; then
         IFS=',' read -r -a safe_cows <<< 'default,default,default,default,bud-frogs,chameleon,elephant,small,three-eyes,tux,rancher,rancher-trademarked,kitten,robotfindskitten,owl,hellokitty'
...
*** Apt
    ...
    #+begin_src conf
      Acquire::http::Proxy "http://172.16.17.4:3128/";
      Acquire::https::Proxy "http://172.16.17.4:3128/";
    #+end_src
*** Everything on openSUSE
    Change the following in =/etc/sysconfig/proxy
    #+begin_src conf
      PROXY_ENABLED="yes"
      HTTP_PROXY="http://172.16.17.4:3128/"
      HTTPS_PROXY="http://172.16.17.4:3128/"
      NO_PROXY="localhost,127.0.0.1,172.16.17.0/24"
    #+end_src
...
** Client setup
   ...
   TODO: figure out where certificates go in openSUSE Tumbleweed

    Create file =/etc/systemd/system/docker.service.d/http-proxy.conf=
    #+begin_src conf
      [Service]
      Environment="HTTP_PROXY=http://172.16.17.4:6128/"
      Environment="HTTPS_PROXY=http://172.16.17.4:6128/"
      Environment="NO_PROXY=localhost,127.0.0.1,docker-registry.example.com,.corp,quay.io"
    #+end_src

    Get the CA certificate from the proxy and make it a trusted root
    #+begin_src bash
      curl http://172.16.17.4:6128/ca.crt | sudo tee /usr/share/ca-certificates/docker_registry_proxy.crt
...
** Clean up after RKE
   #+begin_src bash
     ...
     WCOLL=~/projects/homelab/pdsh/all-nodes.txt pdsh -R ssh 'sudo rm -rfv /var/lib/longhorn/* /data/longhorn/* /etc/cni/*'
