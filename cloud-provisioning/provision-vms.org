...
** Set up GKE cluster
   + full cluster High Memory configuration
     #+begin_src bash
       ...
       KUBECONFIG=~/.kube/config-files/gke-gg-gmk8s.yaml gcloud beta container --project "tigergraph-sales" clusters create "gg-gmk8s" \
              --zone "us-west3-a" \
   ...
   + basic cluster
     #+begin_src bash
       KUBECONFIG=~/.kube/config-files/gke-gg-basic.yaml gcloud beta container --project "tigergraph-sales" clusters create "gg-basic" \
              --zone "us-central1-c" \
              --no-enable-basic-auth \
              --cluster-version "1.20.10-gke.301" \
              --release-channel "regular" \
   ...
   + Nuke said cluster
     #+begin_src bash
       gcloud beta container --project "tigergraph-sales" clusters delete "gg-tester" --zone "us-west3-a"
     #+end_src
*** Add a node pool
    #+begin_src bash
      gcloud beta container --project "tigergraph-sales" node-pools create "bigpool" \
             --cluster "gg-synthea" \
             --zone "us-west1-b" \
...
** Single-node Rancher
  #+begin_src bash
    ...
    gcloud beta compute --project=rancher-dev instances create gg-rancher-25-leap \
           --zone=us-west3-a \
           --machine-type=e2-medium \
           --network=default \
           --address=${IP} \
