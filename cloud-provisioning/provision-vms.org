...
** Set up EKS Cluster
   #+begin_src bash
     ...
     aws eks update-kubeconfig --kubeconfig ~/.kube/config-files/eks-gg-test.yaml --name gg-tiger32-test --region us-west-2
   #+end_src
   #+begin_src bash
...
** Set up AKS cluster
   ...
   + Nab credentials
     #+begin_src bash
       az aks get-credentials --resource-group gg-aks-rg --name gg-tiger32-test --file -
     #+end_src
