...
#+startup: content
* Provision managed Kubernetes in cloud platforms
** Set up EKS Cluster
   #+begin_src bash
     ...
     aws eks update-kubeconfig --kubeconfig ~/.kube/config-files/eks-gg-test.yaml --name gg-tiger35 --region us-west-2
   #+end_src
   #+begin_src bash
     eksctl delete cluster --name gg-tiger --profile <MYPROFILE>
   #+end_src
** Set up AKS cluster
   + ref: [[https://docs.microsoft.com/en-us/azure/aks/kubernetes-walkthrough][Quickstart: Deploy an AKS cluster by using Azure CLI - Azure Kubernetes Servi...]]
     #+begin_src bash
     # create resource group
     az group create --name gg-aks-rg --location eastus
     #+end_src
   ...
   + Create AKS cluster
     ...
     #+end_src
   + Install =kubectl= using Azure (*VERY* OPTIONAL)
     #+begin_src bash
       az aks install-cli
     #+end_src
   + Nab credentials
     #+begin_src bash
       az aks get-credentials --resource-group gg-aks-rg --name gg-tiger35 --file -
     #+end_src
   + Nuke the cluster by deleting the resource group (love Azure)
     #+begin_src bash
       az group delete --name gg-aks-rg --yes --no-wait
     #+end_src
** Set up GKE cluster
   + full cluster High Memory configuration
     #+begin_src bash
                # --cluster-version "1.20.10-gke.301" \
              # --max-pods-per-node "110" \

       KUBECONFIG=~/.kube/config-files/gke-gg-gmk8s.yaml gcloud beta container --project "MYPROJECT" clusters create "gg-gmk8s" \
              --zone "us-west3-a" \
              --labels="owner=greg_grubbs" \
              ...
              --monitoring=SYSTEM \
              --enable-ip-alias \
              --network "projects/tigergraph-sales/global/networks/default" \
              --subnetwork "projects/tigergraph-sales/regions/us-west3/subnetworks/default" \
              ...
              --node-locations "us-west3-a"
     #+end_src
   + basic cluster
     #+begin_src bash
       KUBECONFIG=~/.kube/config-files/gke-gg-basic.yaml gcloud beta container --project "MYPROJECT" clusters create "gg-basic" \
              --zone "us-central1-c" \
              --no-enable-basic-auth \
              --cluster-version "1.20.10-gke.301" \
              --release-channel "regular" \
              ...
              --enable-ip-alias \
              --network "projects/tigergraph-sales/global/networks/default" \
              --subnetwork "projects/tigergraph-sales/regions/us-central1/subnetworks/default" \
              --no-enable-intra-node-visibility \
              --default-max-pods-per-node "110" \
              --no-enable-master-authorized-networks \
              --addons HorizontalPodAutoscaling,HttpLoadBalancing,GcePersistentDiskCsiDriver \
     ...
     #+end_src
   + Private cluster with outbound internet access via CloudNAT
     + ref: [[https://cloud.google.com/nat/docs/gke-example][Example GKE setup  |  Cloud NAT  |  Google Cloud]]

   + gcloud
     #+begin_src bash
       # Create VPC net and subnet
       gcloud compute networks create gg-k8s-private-net \
              --subnet-mode custom
       # Specify subnet prefix for your first region
       gcloud compute networks subnets create subnet-us-east-192 \
              --network gg-k8s-private-net \
              --region us-west3 \
       ...
       gcloud compute routers nats create nat-config \
           --router-region us-west3 \
           --router nat-router \
           --nat-all-subnet-ip-ranges \
           --auto-allocate-nat-external-ips
       # Create the cluster! Private that can access internet
       time KUBECONFIG=~/.kube/config-files/gke-nat-test.yaml gcloud container clusters create gg-tester \
            --zone us-west3-a \
            --labels="owner=greg_grubbs" \
            --no-enable-basic-auth \
            --release-channel "regular" \
            ...
            --disk-type "pd-standard" \
            --disk-size "100" \
            --network "projects/stardog-presales/global/networks/gg-k8s-private-net" \
            --subnetwork "projects/stardog-presales/regions/us-west3/subnetworks/subnet-us-east-192" \
            --no-enable-master-authorized-networks \
            --enable-ip-alias \
   ...
   + Nuke said cluster
     #+begin_src bash
       gcloud beta container --project "MYPROJECT" clusters delete "gg-tester" --zone "us-west3-a"
     #+end_src
*** Add a node pool
    #+begin_src bash
      gcloud beta container --project "MYPROJECT" node-pools create "bigpool" \
             --cluster "gg-synthea" \
             --zone "us-west1-b" \
...
** Single-node Rancher
  #+begin_src bash
    IP=nn.nn.nn.nn # reserved IP
    gcloud beta compute --project=MYPROJECT instances create gg-rancher-25-leap \
           --zone=us-west3-a \
           --machine-type=e2-medium \
           --network=default \
           --address=${IP} \
           ...
           --boot-disk-device-name=gg-rancher-25-leap \
...
** OS options for cluster nodes
   ...
   | ubuntu | ubuntu-2004-lts | ubuntu-os-cloud |
...
*** individual instances vs instance groups
    + Create instance templates
      #+begin_src bash  :var IMAGE_FAMILY=cluster-image-family :var IMAGE_PROJECT=cluster-image-project :results output
        gcloud beta compute --project=MYPROJECT \
...
* Provision k3s for single-node use
  ...
  Like vehicle install - k3s, local path provisioner
   ...
   #+begin_src bash
     KUBECONFIG=~/.kube/config-files/minikube.yaml minikube start
   #+end_src
...
** With k3sup
   ...
   #+end_src
** With k3sup and Calico networking
   #+begin_src bash
     k3sup install  \
           --cluster \
           --context k3s-lab \
           --host pandora \
           --k3s-channel v1.21 \
           --k3s-extra-args="--flannel-backend none --cluster-cidr 192.168.0.0/16 --disable-network-policy --disable traefik,servicelb" \
           --user gregj
     ...
     k3sup join \
           ...
           --user gregj
   #+end_src
** From Adrian's video: k3s+kube-vip+metallb
   + Source video: [[https://www.youtube.com/watch?v=9PLw1xalcYA&t=471s][HA K3s with etcd, kube-vip, MetalLB, and Rancher! - YouTube]]
   + Initial =k3sup= command
    =--tls-san= says our certs should also accept this IP - this will be our VIP
     #+begin_src bash
       k3sup install \
             --host=todd.magichome \
             --user=gregj \
             ...
             --local-path=config.k3slab.yaml \
             --context gg-k3slab \
             --cluster \
             --tls-san=172.16.17.13 \
             --k3s-extra-args="--disable servicelb --node-taint node-role.kubernetes.io/master=true:NoSchedule"
       # --tls-san=10.68.0.240 \
     #+end_src
   + SSH to our new server node to install =kube-vip=
     + Download RBAC config
       #+begin_src bash
         curl -s https://kube-vip.io/manifests/rbac.yaml | \
              sudo tee /var/lib/rancher/k3s/server/manifests/kube-vip-rbac.yaml
       #+end_src
     + Edit the above to add api group
       NOTE: this is no longer necessary - but check for the below config
       #+begin_src yaml
         rules:
       ...
       #+end_src
     + Set env vars for VIP and LAN interface of the host
       #+begin_src bash
         export VIP=172.16.17.13
         export INTERFACE=enx00e04c6815d9
       #+end_src
     + Pull =kube-vip= image
       =kube-vip= will generate its manifest from options provided to the container.
       Therefore, =kube-vip= docs recommend pulling the container then creating an alias
       for that command
       #+begin_src bash
         crictl pull docker.io/plndr/kube-vip:0.3.2
         alias kube-vip="k3s ctr run --rm --net-host docker.io/plndr/kube-vip:0.3.2 vip /kube-vip"
       #+end_src
     + Create the manifest
       We'll run =kube-vip= as a DaemonSet on the control plane nodes.  Announce on Layer
       2 (ARP) using the interface and VIP set earlier. Load balance the control plane and
       utilize Kubernetes leader election to decide which pod makes the
       announcement. Tolerate the taint we put in place for our control planes.  Use the
       service account we had defined in the RBAC step
       #+begin_src bash
       ...
       #+end_src
     + Fix the generated =kube-vip= manifest so that it will actually get scheduled
       #+begin_src yaml
         tolerations:
           - effect: NoSchedule
             key: node-role.kubernete.io/master
             operator: Exists
       #+end_src
     + Update our =kubeconfig= to use the VIP once we can ping the VIP
     + now we can bring up 2 more servers and an agent, pointing them to the VIP
       #+begin_src bash
         k3sup join --host=birdperson.magichome \
               --server-user=gregj \
               --ssh-key ~/.ssh/k8s-local \
               --server-host=${VIP} \
               --user=gregj \
               ...
               --server \
               --k3s-extra-args="--disable servicelb --node-taint=node.role.kubernetes.io/master=true:NoSchedule"
         k3sup join --host=cuphead.magichome \
               --server-user=gregj \
               --ssh-key ~/.ssh/k8s-local \
               --server-host=${VIP} \
               --user=gregj \
         ...
         k3sup join --host=chewbacca.magichome \
               --ssh-key ~/.ssh/k8s-local \
               --server-host=${VIP} \
               --user=gregj \
               --k3s-version=v1.19.4+k3s1

         k3sup join --host=marvin.magichome \
               --ssh-key ~/.ssh/k8s-local \
               --server-host=${VIP} \
               --user=gregj \
               --k3s-version=v1.19.4+k3s1
       #+end_src
     + Failover of the VIP is fast
       If we shutdown k3s on =demo-a= the VIP moves
     + Let's bring up a worker node - that lets =traefik= start!
       #+begin_src bash
         k3sup join --host=demo-b \
               --server-user=root \
               --server-host=${VIP} \
               --user=root \
               --k3s-version=v1.19.4+k3s1 \
               ...
               --k3s-extra-args="--disable servicelb --node-taint=node-role.kubernetes.io/master=true:NoSchedule"
       #+end_src
* Up and Running: k3s - with Adrian Goins
  + [[https://community.suse.com/courses/4522316/feed][Course on SUSE Community]]
  + [[https://community.suse.com/courses/4522316/topics/4563584][Week 2: Networking and Drivers]]
    + [[https://youtu.be/ebbdJVRi4Ng][Up and Running: K3s - Session Two - YouTube]]
    + [[file:Up_and_Running_K3s_-_Topic_Session_2.pdf][slides]]
    + ServiceLB can be selectively included on nodes by adding the label =svccontroller.k3s.cattle.io/enablelb=
    + If the label is present, *only* those hosts will receive the servicelb pods
    +
...
** Using Cloud-Init and Terraform
   Refs:
   + [[https://norocketscience.at/deploy-proxmox-virtual-machines-using-cloud-init/][Deploy Proxmox virtual machines using Cloud-init]]
   + [[https://norocketscience.at/provision-proxmox-virtual-machines-with-terraform/][Install Proxmox virtual machines with Terraform]]
     #+begin_src bash
       sudo qm create 9000 --name "/mnt/pve/gg-images/images/ubuntu-2004-cloudinit-template" --memory 2048 --net0 virtio,bridge=vmbr0
       sudo qm importdisk 9000 ubuntu-20.04-minimal-cloudimg-amd64.qcow2 local-lvm
       ...
       sudo qm clone 9000 142 --name gg-my-vm
       ssh 172.16.17.121 -v
     #+end_src
