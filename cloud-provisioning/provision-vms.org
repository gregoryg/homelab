...
** Set up EKS Cluster
   + Start with a single-node system-only cluster
   #+begin_src bash
   ...
   #+begin_src bash
     eksctl create nodegroup \
       ...
       --nodes 3
   #+end_src
   + To target the nodepool in Kubernetes manifests, use
     #+begin_src yaml
       nodeSelector:
         eks.amazonaws.com/nodegroup: gg-starpool
     #+end_src
   + To every season turn, turn turn
   #+begin_src bash
     eksctl delete cluster --name gg-stardog --profile <MYPROFILE>
   #+end_src
** Set up AKS cluster
   ...
   + Create AKS cluster
     #+begin_src bash
       ...
       az aks create \
          ...
          --ssh-key-value ~/.ssh/gg-azure-stardog.pub \
   ...
   + Nab credentials
     #+begin_src bash
       az aks get-credentials --resource-group gg-aks-rg --name gg-stardog --file -
     #+end_src
